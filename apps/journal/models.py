...
class Entry(TranslatableModel):
    ...
    def get_relative_url_archive_index(self):
        logger.debug('1get_relative_url_archive_index: data:%s slug:%s' % (self.pub_date.strftime('%Y/%m/%d'), self.slug))
        return '%s/%s/' % (self.pub_date.strftime('%Y/%m/%d'), self.slug)

    def get_relative_url_archive_year(self):
        logger.debug('1get_relative_url_archive_year: data:%s slug:%s' % (self.pub_date.strftime('%m/%d'), self.slug))
        return '%s/%s/' % (self.pub_date.strftime('%m/%d'), self.slug)

    def get_relative_url_archive_month(self):
        logger.debug('1get_relative_url_archive_month: data:%s slug:%s' % (self.pub_date.strftime('%d'), self.slug))
        return '%s/%s/' % (self.pub_date.strftime('%d'), self.slug)

    def get_relative_url_archive_day(self):
        logger.debug('1get_relative_url_archive_day: slug:%s' % (self.slug))
        return '%s/' % self.slug

    def get_relative_url(self):
        logger.debug('get_relative_url')
